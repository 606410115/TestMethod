...
public class BleBroadcastReceiver{
    ...
    public BleBroadcastReceiver(Activity activity, TimeManager tM){
        timeManager = tM;
    ...
    public static MOTiLogActivity mLogActivity = new MOTiLogActivity(timeManager);
            ...
            public void onReceive(Context context,final Intent intent) {
                ...
                if (BluetoothLeService.ACTION_GATT_CONNECTED.equals(action)) {
                    if(currentActivity instanceof MainActivity)
                        ((MainActivity)currentActivity).connect();
                    ...
                    new Handler().postDelayed(new Runnable() {
                        ...
                        public void run() {
                            ...
                            if(currentActivity instanceof MainActivity)
                                ((MainActivity)currentActivity).disconnect(intent.getStringExtra("name"));
                        }
                    }, 600);
                } else if (BluetoothLeService.ACTION_GATT_SERVICES_DISCOVERED.equals(action)) {
                    Log.d(TAG,intent.getStringExtra("name") + "  Discover service success");
                } else if (BluetoothLeService.ACTION_DATA_AVAILABLE.equals(action)) {
                    mLogActivity.addLogData(intent.getByteArrayExtra(BluetoothLeService.EXTRA_DATA), intent.getStringExtra("name"));
                }
            }
        };
